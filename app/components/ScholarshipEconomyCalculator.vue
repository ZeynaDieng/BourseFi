  ...
  <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-premium">
    ...
    <dl class="space-y-3 text-sm">
      <div class="flex justify-between gap-4 border-b border-slate-50 pb-3">
        <dt class="text-slate-500">Frais de dossier (référentiel)</dt>
    ...
    <p v-if="fraisDossierEtranger && fraisDossierEtranger > 0" class="mt-4 text-xs text-slate-400">
      Frais de dossier étrangers : {{ fraisDossierEtranger.toLocaleString('fr-FR') }} {{ devise ?? 'FCFA' }}
    </p>
  </div>
</template>
