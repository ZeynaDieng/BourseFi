...
const { data: paiements, pending: pendingPaiements } = await useFetch('/api/paiements', { lazy: true, server: false })
const { data: candidatures, pending: pendingCandidaturesFetch } = await useFetch('/api/candidatures', { lazy: true, server: false })

const isLoading = computed(() => pendingPaiements.value || pendingCandidaturesFetch.value)
  ...
  <main class="min-h-[calc(100dvh-4rem)] bg-gradient-to-b from-primary/[0.04] to-background pb-8">
    <StudentPageShell title="Mes paiements" subtitle="Frais de dossier et reçus.">
      ...
      <div v-if="isLoading" class="space-y-4">
        <AppSkeleton type="table" :count="2" />
      </div>

      <div v-else>
        <section v-if="pendingCandidatures.length" class="mb-6 animate-fade-in-up">
          <h2 class="mb-3 text-xs font-bold uppercase tracking-widest text-amber-700">À régler</h2>
          <ul class="space-y-3">
            <li
              v-for="c in pendingCandidatures"
              :key="c.id"
              class="rounded-2xl border border-amber-100 bg-amber-50/60 p-4 shadow-sm"
            >
        ...
        <ul v-if="paiements?.length" class="space-y-3">
          <li
            v-for="p in paiements"
            :key="p.id"
            class="rounded-2xl border border-slate-100 bg-white p-4 shadow-premium animate-fade-in-up"
          >
        ...
        <div
          ...
          class="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-12 text-center animate-fade-in-up"
        >
