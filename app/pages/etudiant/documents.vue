...
const { data: me, pending: pendingMe } = await useFetch('/api/auth/me', { lazy: true, server: false })
const { data: candidatures, pending: pendingCandidatures } = await useFetch('/api/candidatures', { lazy: true, server: false })
const { data: paiements, pending: pendingPaiements } = await useFetch('/api/paiements', { lazy: true, server: false })

const isLoading = computed(() => pendingMe.value || pendingCandidatures.value || pendingPaiements.value)
